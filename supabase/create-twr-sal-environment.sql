-- TWR SAL · Setup base do ambiente
--
...
begin
  ...
  if unit_type_data_type = 'USER-DEFINED' then
    execute format(
      $sql$
        ...
        values ('Amilcar Cabral Tower', 'TWR_SAL', %L::%I.%I)
    ...
    insert into public.ats_units (name, code, unit_type)
    values ('Amilcar Cabral Tower', 'TWR_SAL', target_unit_type)
    on conflict (code) do update
    set
      name = excluded.name,
      unit_type = excluded.unit_type;
  ...
  if target_ats_unit_id is null then
    raise exception 'ATS unit with code % was not found.', 'TWR_SAL';
  end if;
end $$;

-- Horários operacionais da TWR SAL (UTC)
-- 07:30 - 13:30
-- 13:30 - 19:30
-- 19:30 - 23:30
-- 23:30 - 07:30
